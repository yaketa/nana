...
test("読みは「ぶんくいとうげ」（く＝濁点なし）で統一されている", () => {
  // 正式な読みは「ぶんくい」。現地の道路標識（長野県設置）は "Bunkuitoge"、
  // 伊那市公式ページの見出しも「分杭峠（ぶんくいとうげ）」、
  // 日本歴史地名大系も「ぶんくいとうげ」。
  // 「ぶんぐい」は世間に広く出回っている通称（Wikipedia もこちら）なので、
  // 書きたすときにうっかり混ざりやすい。ここで止める。
  const readme = fs.readFileSync(path.join(__dirname, "..", "README.md"), "utf8");
  for (const [name, text] of [["index.html", html], ["README.md", readme]]) {
    assert.ok(!/ぶんぐい/.test(text), `${name} に「ぶんぐい」が残っている（正しくは「ぶんくい」）`);
  }
  assert.match(html, /<rt>ぶんくいとうげ<\/rt>/, "題字のふりがなが「ぶんくいとうげ」でない");
  assert.match(html, /<meta name="description"[^>]*分杭峠（ぶんくいとうげ）/,
    "検索結果に出る説明文のふりがなが「ぶんくいとうげ」でない");

  // 自分たちで名前を決められるもの（写真のファイル名）も濁点なしにそろえる。
  // 外部サイトの URL は先方の綴りなので、ここでは見ない。
  for (const img of images) {
    for (const ref of [img.src, ...img.srcset]) {
      assert.ok(!/bungui/i.test(ref), `写真のファイル名に bungui が残っている: ${ref}`);
    }
  }
});
...
test("表紙のふりがなは、ブラウザ標準のルビ配置のまま。位置ずらしの指定を足さない", () => {
  // ふりがな（ぶんくいとうげ）が題字の右へ流れ出て並んでしまった不具合の再発防止。
  ...
  assert.match(html, /<h1><ruby><span class="h1-txt">分杭峠<\/span><rt>ぶんくいとうげ<\/rt><\/ruby><\/h1>/,
